...
#define SYNA_TCM_CORE_LIB_VERSION 0x011d
 ...
 *
  * @brief: MAX_SIZE_GRIP_INFO
 *         Maximum size of customer Grip Info
 */
...
#define RST_ENABLE_IRQ_DELAY_MS       15
...
enum dynamic_tcm_config_id {
	...
	DC_CONTROL_LBP_HBP = 0x11, /* 2-LBP(default),1-HBP */
	DC_TOUCH_AND_HOLD = 0xD4,
	DC_FREQUENCE_HOPPING_SYNA = 0xD2,
	DC_LOW_TEMP_MODE_ENABLE = 0xFD,
	DC_GESTURE_TYPE_ENABLE = 0xFE, /* Bit0 -- Double Tap, Bit13 -- Single Tap */
	DC_ALL = 0xFF,
};
...
/* power state is used for TCM/TSM */
struct tcm_power_state_info {
	/* power state */
	unsigned short power_state;
	unsigned short driver_req_mode;
	unsigned short lpwg_enabled;
	unsigned short irq_enabled;
	unsigned short reserved[4];
};

/* driver state is used for tsDaemon to sync signal */
struct tcm_driver_state_info {
	unsigned short power_state;  /* power state */
	unsigned short hw_resetting; /* 0 - hw reset done; 1 - hw resetting */
	unsigned short lpwg_enabled; /* 0 - disabled; 1 - enabled */
	unsigned short irq_enabled;  /* 0 - disabled, 1 - enabled */
};
